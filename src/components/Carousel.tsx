...
export default function Carousel() {
  ...
  return (
    <div className="relative w-screen overflow-hidden">
      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex">
          {slides.map((slide, index) => (
            <div
              key={index}
              className="flex-[0_0_100%] w-screen relative h-[70vh]"
            >
              <Image
                src={slide.src}
                alt={slide.title}
      ...
      <button
        onClick={scrollPrev}
        className="absolute top-1/2 left-5 -translate-y-1/2 bg-black/40 hover:bg-black/60 text-white p-3 rounded-full z-20"
      >
        <ChevronLeft size={28} />
      </button>
      ...
      <button
        onClick={scrollNext}
        className="absolute top-1/2 right-5 -translate-y-1/2 bg-black/40 hover:bg-black/60 text-white p-3 rounded-full z-20"
      >
