// src/data/bebidas.ts
export type TextoBilíngue = {
  pt: string;
  en: string;
};

export type Bebida = {
  titulo: TextoBilíngue;
  descricao: TextoBilíngue;
  preco: string | string[] | TextoBilíngue[];
  imagem: string;
};

export const chopps: Bebida[] = [
  {
    titulo: {
      pt: "Chopp Heineken",
      en: "Heineken",
    },
    descricao: {
      pt: "Chopp premium, com ingredientes puros e selecionados.",
      en: "",
    },
    preco: "R$ 12,50",
    imagem: "/chopps/chopp-heineken.webp",
  },
  {
    titulo: {
      pt: "Chopp Antarctica",
      en: "Antarctica",
    },
    descricao: {
      pt: "Chopp fresco, aroma neutro. Para quem prefere chopp com teor de amargor menos acentuado.",
      en: "",
    },
    preco: "R$ 9,90",
    imagem: "/chopps/chopp-antarctica.webp",
  },
  {
    titulo: {
      pt: "Chopp Brahma Black",
      en: "Brahma Black Beer",
    },
    descricao: {
      pt: "Cremoso e surpreendente.  A escolha certa para quem gosta de bebidas marcantes.​",
      en: "",
    },
    preco: "R$ 12,90",
    imagem: "/chopps/chopp-black.webp",
  },
  {
    titulo: {
      pt: "Chopp Ferrugem",
      en: "Rust",
    },
    descricao: {
      pt: "Grande novidade! Chopp Antarctica, com 3 dedos de creme black na tulipa Gerê.",
      en: "Antarctica beer with three fingers of black beer.",
    },
    preco: "R$ 10,90",
    imagem: "/chopps/chopp-ferrugem.webp",
  },
  {
    titulo: {
      pt: "Chopp Amstel",
      en: "Amstel",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 9,90",
    imagem: "/chopps/chopp-amstel.webp",
  },
];

export const cervejas600: Bebida[] = [
  {
    titulo: {
      pt: "Heineken",
      en: "Heineken",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 19,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Amstel",
      en: "Amstel",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 15,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Original",
      en: "Original",
    },
    descricao: {
      pt: "​",
      en: "",
    },
    preco: "R$ 17,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Baden baden ipa",
      en: "Baden baden ipa",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 23,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Limão espremido",
      en: "Lime (squeezed)",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 4,00",
    imagem: "/null.webp",
  },
];

export const longNecks: Bebida[] = [
  {
    titulo: {
      pt: "Amstel Ultra - Sem Glúten",
      en: "Amstel Ultra - gluten free",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 10,00",
    imagem: "/longnecks/amstel-ultra.webp",
  },
  {
    titulo: {
      pt: "Blue Moon",
      en: "Blue Moon",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 23,00",
    imagem: "/longnecks/blue-moon.webp",
  },
  {
    titulo: {
      pt: "Stella Artois Pure Gold",
      en: "Stella Artois Pure Gold",
    },
    descricao: {
      pt: "Sem glúten.​",
      en: "Gluten Free",
    },
    preco: "R$ 14,00",
    imagem: "/longnecks/stella-pure-gold.webp",
  },
  {
    titulo: {
      pt: "Heineken 330 ml",
      en: "Heineken 330 ml",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/longnecks/heineken-long-neck.webp",
  },
  {
    titulo: {
      pt: "Corona 330 ml",
      en: "Corona 330 ml",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/longnecks/corona.webp",
  },
  {
    titulo: {
      pt: "Heineken 330 ml – Zero álcool",
      en: "Heineken 0.0",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/longnecks/heineken-zero-long-neck.webp",
  },
  {
    titulo: {
      pt: "Corona zero",
      en: "Corona 0.0",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/null.webp",
  },
];

export const drinks: Bebida[] = [
  {
    titulo: {
      pt: "Carmen Citrus",
      en: "Carmen Citrus",
    },
    descricao: {
      pt: "Carmen Citrus é uma homenagem a nossa especial chef de cozinha Carminha. Esse drink leve, frutado e aromático, foi inserido em homenagem ao dia das mulheres. Possui um toque de baunilha, morango e limão siciliano.",
      en: "Carmen Citrus is a tribute to our special chef Carminha. This light, fruity, and aromatic drink was created in honor of International Women’s Day. It features notes of vanilla, strawberry, and Sicilian lemon.",
    },
    preco: [
      { pt: "Cachaça: R$ 23,00", en: "Cachaça: R$ 23.00" },
      { pt: "Vodka nacional: R$ 26,00", en: "National Vodka: R$ 26.00" },
      { pt: "Vodka importada:  R$ 31,00", en: "Imported Vodka: R$ 31.00" },
    ],
    imagem: "/drinks/carmem-citrus.webp",
  },
  {
    titulo: {
      pt: "Juju Tropical",
      en: "Juju Tropical",
    },
    descricao: {
      pt: "Para quem quer celebrar com um drink cítrico e frutado, Juju tropical foi acrescentado no cardápio para homenagear nossa grande Judith. Um mistura de pitaya, maracujá, hortelã.",
      en: "For those who want to celebrate with a citrusy and fruity drink, Juju Tropical was added to the menu as a tribute to our dear Judith. A mix of pitaya, passion fruit, and mint.",
    },
    preco: [
      { pt: "Cachaça: R$ 23,00", en: "Cachaça: R$ 23.00" },
      { pt: "Vodka nacional: R$ 26,00", en: "National Vodka: R$ 26.00" },
      { pt: "Vodka importada:  R$ 31,00", en: "Imported Vodka: R$ 31.00" },
    ],
    imagem: "/drinks/juju-tropical.webp",
  },
  {
    titulo: {
      pt: "Lili Twist",
      en: "Lili Twist",
    },
    descricao: {
      pt: "E para quem adora um drink com o frescor de gengibre,  criamos um drink Lili Twist. Uma mistura de abacaxi, limão, própolis e espuma de gengibre. Em especial a nossa querida Liliane!​",
      en: "And for those who love a drink with the freshness of ginger, we created the Lili Twist — a blend of pineapple, lemon, propolis, and ginger foam. A special tribute to our beloved Liliane!",
    },
    preco: "R$ 26,00",
    imagem: "/drinks/lili-twist.webp",
  },
  {
    titulo: {
      pt: "Fitzgeria",
      en: "Fitzgeria",
    },
    descricao: {
      pt: "Gin, limão siciliano, Angostura Bitter, água com gás e xarope de açúcar.",
      en: "Gin, Sicilian lemon, Angostura Bitter, sparkling water, and sugar syrup.",
    },
    preco: [
      { pt: "Gin nacional: R$ 29,00", en: "National Gin: R$ 29.00" },
      { pt: "Gin importado: R$ 34,00", en: "Imported Gin: R$ 34.00" },
    ],
    imagem: "/drinks/fitzgeria.webp",
  },
  {
    titulo: {
      pt: "Rabo de saia",
      en: "Rabo de saia",
    },
    descricao: {
      pt: "Cynar, Cachaça, Jägermeister, Angostura, laranja, melaço de cana e limão siciliano.",
      en: "Cynar, cachaça, Jägermeister, Angostura, orange, cane molasses, and Sicilian lemon.",
    },
    preco: "R$ 29,00",
    imagem: "/drinks/rabo-de-saia.webp",
  },
  {
    titulo: {
      pt: "Caipi Gerê",
      en: "Caipi Gerê",
    },
    descricao: {
      pt: "Xarope de rapadura, limão taiti.",
      en: "Brown sugar syrup and Tahiti lime.",
    },
    preco: [
      { pt: "Cachaça: R$ 25,00", en: "Cachaça: R$ 25.00" },
      { pt: "Vodka nacional: R$ 28,00", en: "National Vodka: R$ 28.00" },
      { pt: "Vodka importada:  R$ 33,00", en: "Imported Vodka: R$ 33.00" },
    ],
    imagem: "/drinks/caipi-gere.webp",
  },
  {
    titulo: {
      pt: "Gerê tropical",
      en: "Gerê Tropical",
    },
    descricao: {
      pt: "Nosso Geremia tropical, é uma bebida refrescante com dose de gin Beefeater, maracujá e energético Red Bull tropical.",
      en: "Our tropical Geremia — a refreshing drink made with a shot of Beefeater gin, passion fruit, and Red Bull Tropical energy drink.",
    },
    preco: [
      { pt: "Gin nacional: R$ 34,00", en: "National Gin: R$ 34.00" },
      { pt: "Gin importado: R$ 39,00", en: "Imported Gin: R$ 39.00" },
    ],
    imagem: "/drinks/gere-tropical.webp",
  },
  {
    titulo: {
      pt: "Caraíva",
      en: "Caraíva",
    },
    descricao: {
      pt: "Drink com morango, maracujá e tangerina.",
      en: "Drink with strawberry, passion fruit, and tangerine.",
    },
    preco: [
      { pt: "Cachaça: R$ 26,00", en: "Cachaça: R$ 26.00" },
      { pt: "Vodka nacional: R$ 29,00", en: "National Vodka: R$ 29.00" },
      { pt: "Vodka importada:  R$ 34,00", en: "Imported Vodka: R$ 34.00" },
    ],
    imagem: "/drinks/caraiva.webp",
  },
  {
    titulo: {
      pt: "Maracujack",
      en: "Maracujack",
    },
    descricao: {
      pt: "Maracujack, será um queridinho do Gerê. Uma mistura de dose de Jack Daniel's e polpa de maracujá.",
      en: "A future favorite at Gerê — a mix of Jack Daniel’s whiskey and passion fruit pulp.",
    },
    preco: "R$ 29,00",
    imagem: "/drinks/maracujack.webp",
  },
  {
    titulo: {
      pt: "Mojito",
      en: "Mojito",
    },
    descricao: {
      pt: "Famoso Mojito, um coquetel a base de rum, água com gás, limão e hortelã.",
      en: "A cocktail made with rum, sparkling water, lime, and mint.",
    },
    preco: "R$ 25,00",
    imagem: "/drinks/mojito.webp",
  },
  {
    titulo: {
      pt: "Aperol Rosê",
      en: "Aperol Rosê",
    },
    descricao: {
      pt: "Aperol, espumante brut, suco de laranja, limão siciliano e xarope de morango.",
      en: "Aperol, brut sparkling wine, orange juice, Sicilian lemon, and strawberry syrup.",
    },
    preco: "R$ 29,00",
    imagem: "/drinks/aperol-rose.webp",
  },
  {
    titulo: {
      pt: "Negroni",
      en: "Negroni",
    },
    descricao: {
      pt: "O clássico Negroni que dispensa comentários.",
      en: "The classic Negroni — a timeless favorite that needs no introduction.",
    },
    preco: [
      { pt: "Gin nacional: R$ 29,00", en: "National Gin: R$ 29.00" },
      { pt: "Gin importado: R$ 34,00", en: "Imported Gin: R$ 34.00" },
    ],
    imagem: "/drinks/negroni.webp",
  },
  {
    titulo: {
      pt: "Moscow Mule",
      en: "Moscow Mule",
    },
    descricao: {
      pt: "Vodka, espuma e xarope de gengibre e limão.",
      en: "Vodka with ginger and lemon syrup and foam.",
    },
    preco: [
      { pt: "Vodka nacional: R$ 29,00", en: "National Vodka: R$ 29.00" },
      { pt: "Vodka importado: R$ 34,00", en: "Imported Vodka: R$ 34.00" },
    ],
    imagem: "/drinks/moscow-mule.webp",
  },
  {
    titulo: {
      pt: "Gin tonica com morango",
      en: "Strawberry Gin tonic",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: [
      { pt: "Gin nacional: R$ 29,00", en: "National Gin: R$ 29.00" },
      { pt: "Gin importado: R$ 34,00", en: "Imported Gin: R$ 34.00" },
    ],
    imagem: "/drinks/gin-com-morango.webp",
  },
  {
    titulo: {
      pt: "Sicilian bee",
      en: "Sicilian Bee",
    },
    descricao: {
      pt: "Gin, tonica, limão siciliano e mel.",
      en: "Gin, tonic, lemon, honey",
    },
    preco: [
      { pt: "Gin nacional: R$ 29,00", en: "National Gin: R$ 29.00" },
      { pt: "Gin importado: R$ 34,00", en: "Imported Gin: R$ 34.00" },
    ],
    imagem: "/drinks/sicilian-bee.webp",
  },
  {
    titulo: {
      pt: "Aperol Spritz",
      en: "Aperol Spritz",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 29,00",
    imagem: "/drinks/aperol-spritz.webp",
  },
  {
    titulo: {
      pt: "Cozumel",
      en: "Cozumel",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Cozumel Corona",
      en: "Cozumel Corona",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 17,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Cozumel Corona zero",
      en: "Cozumel Corona 0.0",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 17,00",
    imagem: "/drinks/cozumel-corona-zero.webp",
  },
  {
    titulo: {
      pt: "Cozumel Heineken",
      en: "Cozumel Heineken",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 17,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Cozumel Heineken zero",
      en: "Cozumel Heineken 0.0",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 17,00",
    imagem: "/drinks/cozumel-heineken.webp",
  },
  {
    titulo: {
      pt: "Cozumel Amstel Ultra",
      en: "Cozumel Amstel Ultra",
    },
    descricao: {
      pt: "Sem glúten.",
      en: "Gluten free",
    },
    preco: "R$ 14,00",
    imagem: "/drinks/cozumel-amstel-ultra.webp",
  },
  {
    titulo: {
      pt: "Licor 43 coffe",
      en: "Licor 43 coffe",
    },
    descricao: {
      pt: "Cuarenta y tres",
      en: "Cuarenta y tres",
    },
    preco: "34,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Ura – sem álcool",
      en: "Ura – Non-alcoholic",
    },
    descricao: {
      pt: "Drink sem álcool à base de abacaxi, xarope de morango e água com gás.",
      en: "A non-alcoholic drink made with pineapple, strawberry syrup, and sparkling water.",
    },
    preco: "R$ 20,00",
    imagem: "/drinks/ura-sem-alcool.webp",
  },
  {
    titulo: {
      pt: "Soda italiana – sem álcool",
      en: "Italian Soda – Non-alcoholic",
    },
    descricao: {
      pt: "Drink sem álcool à base de xarope de frutas, suco de limão e água com gás. Opções de sabores: Morango, Tangerina ou Maça verde.",
      en: "A non-alcoholic drink made with fruit syrup, lime juice, and sparkling water. Flavors available: Strawberry, Tangerine, or Green Apple.",
    },
    preco: "R$ 20,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Suco verde",
      en: "Green Juice",
    },
    descricao: {
      pt: "Suco verde refrescante. Aquela mistura de limão, manjericão, couve, abacaxi, gengibre e água de coco.",
      en: "A blend of lime, basil, kale, pineapple, ginger, and coconut water.",
    },
    preco: "R$ 15,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Suco de pitaya com abacaxi",
      en: "Pitaya Juice with Pineapple",
    },
    descricao: {
      pt: "Mistura de pitaya, abacaxi, hortelã, gengibre, limão e água de coco.",
      en: "A blend of pitaya, pineapple, mint, ginger, lime, and coconut water.",
    },
    preco: "R$ 18,00",
    imagem: "/null.webp",
  },
];

export const caips = [
  {
    titulo: "Caip Seleta",
    descricao: "",
    preco: [
      { pt: "Limão R$ 20,00", en: "Lime R$ 20.00" },
      { pt: "Morango R$ 20,00", en: "Strawberry R$ 20.00" },
      { pt: "Abacaxi R$ 20,00", en: "Pineapple R$ 20.00" },
      { pt: "Kiwi R$ 23,00", en: "Kiwi R$ 23.00" },
      {
        pt: "Morango e maracujá R$ 25,00",
        en: "Strawberry and Passion Fruit R$ 25.00",
      },
      {
        pt: "Limão e maracujá R$ 25,00",
        en: "Lime and Passion Fruit R$ 25.00",
      },
      { pt: "Kiwi e morango R$ 25,00", en: "Kiwi and Strawberry R$ 25.00" },
    ],
    imagem: "/null.webp",
  },
  {
    titulo: "Caipirinha",
    descricao: "",
    preco: [
      { pt: "Limão R$ 18,00", en: "Lime R$ 18.00" },
      { pt: "Morango R$ 18,00", en: "Strawberry R$ 18.00" },
      { pt: "Abacaxi R$ 18,00", en: "Pineapple R$ 18.00" },
      { pt: "Kiwi R$ 21,00", en: "Kiwi R$ 21.00" },
      {
        pt: "Morango e maracujá R$ 23,00",
        en: "Strawberry and Passion Fruit R$ 23.00",
      },
      {
        pt: "Limão e maracujá R$ 23,00",
        en: "Lime and Passion Fruit R$ 23.00",
      },
      { pt: "Kiwi e morango R$ 23,00", en: "Kiwi and Strawberry R$ 23.00" },
    ],
    imagem: "/null.webp",
  },
  {
    titulo: "Caiproska",
    descricao: "",
    preco: [
      { pt: "Limão R$ 20,00", en: "Lime R$ 20.00" },
      { pt: "Morango R$ 20,00", en: "Strawberry R$ 20.00" },
      { pt: "Abacaxi R$ 20,00", en: "Pineapple R$ 20.00" },
      { pt: "Kiwi R$ 23,00", en: "Kiwi R$ 23.00" },
      {
        pt: "Morango e maracujá R$ 25,00",
        en: "Strawberry and Passion Fruit R$ 25.00",
      },
      {
        pt: "Limão e maracujá R$ 25,00",
        en: "Lime and Passion Fruit R$ 25.00",
      },
      { pt: "Kiwi e morango R$ 25,00", en: "Kiwi and Strawberry R$ 25.00" },
    ],
    imagem: "/null.webp",
  },
  {
    titulo: "Caip Absolut",
    descricao: "",
    preco: [
      { pt: "Limão R$ 26,00", en: "Lime R$ 26.00" },
      { pt: "Morango R$ 26,00", en: "Strawberry R$ 26.00" },
      { pt: "Abacaxi R$ 26,00", en: "Pineapple R$ 26.00" },
      { pt: "Kiwi R$ 29,00", en: "Kiwi R$ 29.00" },
      {
        pt: "Morango e maracujá R$ 31,00",
        en: "Strawberry and Passion Fruit R$ 31.00",
      },
      {
        pt: "Limão e maracujá R$ 31,00",
        en: "Lime and Passion Fruit R$ 31.00",
      },
      { pt: "Kiwi e morango R$ 31,00", en: "Kiwi and Strawberry R$ 31.00" },
    ],
    imagem: "/null.webp",
  },
  {
    titulo: "Caip Saquê",
    descricao: "",
    preco: [
      { pt: "Limão R$ 20,00", en: "Lime R$ 20.00" },
      { pt: "Morango R$ 20,00", en: "Strawberry R$ 20.00" },
      { pt: "Abacaxi R$ 20,00", en: "Pineapple R$ 20.00" },
      { pt: "Kiwi R$ 23,00", en: "Kiwi R$ 23.00" },
      {
        pt: "Morango e maracujá R$ 25,00",
        en: "Strawberry and Passion Fruit R$ 25.00",
      },
      {
        pt: "Limão e maracujá R$ 25,00",
        en: "Lime and Passion Fruit R$ 25.00",
      },
      { pt: "Kiwi e morango R$ 25,00", en: "Kiwi and Strawberry R$ 25.00" },
    ],
    imagem: "/null.webp",
  },
];

export const sucos: Bebida[] = [
  {
    titulo: {
      pt: "Laranja",
      en: "Orange Juice",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 12,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Limão",
      en: "Lemonade",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 12,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Abacaxi",
      en: "Pinapple juice",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 12,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Acerola",
      en: "Acerola",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 12,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Maracujá",
      en: "Passion fruit Juice",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 12,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Morango",
      en: "Strawberry juice",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 12,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Abacaxi com hortelã",
      en: "Mix of pineapple and mint juice",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 12,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Laranja com acerola",
      en: "Mix of orange and acerola juice",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Laranja com morango",
      en: "Mix of orange and strawberry juice",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Morango suíço",
      en: "Strawberry (made with milk)",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 16,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Maracujá suíço",
      en: "Passion fruit (made with milk)",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 16,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Limonada suíça",
      en: "Lemonade (made with milk)",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 16,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Suco verde",
      en: "Green Juice",
    },
    descricao: {
      pt: "Suco verde refrescante. Aquela mistura de limão, manjericão, couve, abacaxi, gengibre e água de coco.",
      en: "A blend of lime, basil, kale, pineapple, ginger, and coconut water.",
    },
    preco: "R$ 15,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Suco de pitaya com abacaxi",
      en: "Pitaya Juice with Pineapple",
    },
    descricao: {
      pt: "Mistura de pitaya, abacaxi, hortelã, gengibre, limão e água de coco.",
      en: "A blend of pitaya, pineapple, mint, ginger, lime, and coconut water.",
    },
    preco: "R$ 18,00",
    imagem: "/null.webp",
  },
];

export const zeroAlcool: Bebida[] = [
  {
    titulo: {
      pt: "Água sem gás",
      en: "Still water",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 6,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Água com gás",
      en: "Sparkling water",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 6,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Coca Cola",
      en: "Coke",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 7,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Coca Cola Zero",
      en: "Zero coke",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 7,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Sprite",
      en: "Sprite",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 7,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Schweppes Citrus",
      en: "Schweppes Citrus",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 7,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Guaraná Antártica",
      en: "Guaraná Antártica",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 7,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Guaraná Antártica Zero",
      en: "Zero Guaraná Antártica",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 7,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Tônica",
      en: "Tonic water",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 7,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "H2O! Limão ou Limoneto",
      en: "H20!",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 8,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Red Bull",
      en: "Red Bull",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 18,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Água de coco",
      en: "Coconut water",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 8,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Ura - sem álcool",
      en: "Ura – Non-alcoholic",
    },
    descricao: {
      pt: "Drink sem álcool à base de abacaxi, xarope de morango e água com gás.",
      en: "A non-alcoholic drink made with pineapple, strawberry syrup, and sparkling water.",
    },
    preco: "R$ 20,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Soda italiana – sem álcool",
      en: "Italian Soda – Non-alcoholic",
    },
    descricao: {
      pt: "Drink sem álcool à base de xarope de frutas, suco de limão e água com gás. <br/> Opções de sabores: Morango, Tangerina ou Maça verde.",
      en: "A non-alcoholic drink made with fruit syrup, lime juice, and sparkling water. Flavors available: Strawberry, Tangerine, or Green Apple.",
    },
    preco: "R$ 20,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Heineken 330 ml – Zero álcool",
      en: "Heineken 0.0",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/longnecks/heineken-zero-long-neck.webp",
  },
  {
    titulo: {
      pt: "Corona zero",
      en: "Corona 0.0",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 15,00",
    imagem: "/null.webp",
  },
];

export const cachacaDose: Bebida[] = [
  {
    titulo: {
      pt: "Cachaça Geremia Ouro",
      en: "Cachaça Geremia Gold",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 15,00",
    imagem: "/destilados/souvenir-cachaca-ouro.webp",
  },
  {
    titulo: {
      pt: "Cachaça Geremia Prata",
      en: "Cachaça Geremia Silver",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 12,00",
    imagem: "/destilados/souvenir-cachaca-prata.webp",
  },
  {
    titulo: {
      pt: "Terras de Kubera Ouro",
      en: "Terras de Kubera Gold",
    },
    descricao: {
      pt: "Blended cachaça Bertholletia Ouro",
      en: "Made in Uberaba. It's a Gold (the best) cachaça",
    },
    preco: "R$ 22,00",
    imagem: "/destilados/terras-de-kubera-ouro.webp",
  },
  {
    titulo: {
      pt: "Mata Velha",
      en: "Mata Velha",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 22,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Seleta",
      en: "Seleta",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 15,00",
    imagem: "/null.webp",
  },
];

export const vinhos: Bebida[] = [
  {
    titulo: {
      pt: "El Raco",
      en: "El Raco",
    },
    descricao: {
      pt: "Cabernet sauvignon. Vinho chileno - 750 ml.",
      en: "Cabernet Sauvignon. Chilean wine - 750 ml.",
    },
    preco: "R$ 90,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Alamos",
      en: "Alamos",
    },
    descricao: {
      pt: "Malbec, Mendonza Argentina - 750 ml.",
      en: "Malbec, Mendonza Argentina - 750 ml.",
    },
    preco: "R$ 120,00",
    imagem: "/vinhos/alamos.webp",
  },
  {
    titulo: {
      pt: "La linda",
      en: "La Linda",
    },
    descricao: {
      pt: "Malbec. Argentina - 750 ml.",
      en: "Malbec. Argentina - 750 ml.",
    },
    preco: "R$ 110,00",
    imagem: "/vinhos/la-linda.webp",
  },
  {
    titulo: {
      pt: "Cordero con piel lobo",
      en: "Cordero con piel de lobo",
    },
    descricao: {
      pt: "Malbec. Argentina - 750 ml.",
      en: "Malbec. Argentina - 750 ml.",
    },
    preco: "R$ 120,00",
    imagem: "/vinhos/cordero.webp",
  },
  {
    titulo: {
      pt: "Casillero Del Diablo",
      en: "Casillero Del Diablo",
    },
    descricao: {
      pt: "Carmenere. Vinho chileno - 750 ml.",
      en: "Cabernet sauvignon. Chile - 750 ml.",
    },
    preco: "R$ 110,00",
    imagem: "/vinhos/casillero-del-diablo.webp",
  },
  {
    titulo: {
      pt: "D.V Catena",
      en: "D.V Catena",
    },
    descricao: {
      pt: "Malbec. Mendoza Argentina - 750 ml.",
      en: "Malbec. Mendoza Argentina - 750 ml.",
    },
    preco: "R$ 200,00",
    imagem: "/vinhos/dv-catena.webp",
  },
  {
    titulo: {
      pt: "Casa Perini – Demi sec",
      en: "Casa Perini - Demi sec",
    },
    descricao: {
      pt: "Espumante - Demi-Sec",
      en: "Ssparkling wine - Demi-Sec",
    },
    preco: "R$ 110,00",
    imagem: "/vinhos/casa-perini-demi.webp",
  },
  {
    titulo: {
      pt: "Casa Perini – Moscatel",
      en: "Casa Perini – Moscatel",
    },
    descricao: {
      pt: "Espumante - Moscatel",
      en: "Brazil - Moscatel",
    },
    preco: "R$ 110,00",
    imagem: "/vinhos/casa-perini-moscatel.webp",
  },
  {
    titulo: {
      pt: "Casa Perini – Brut Rosê",
      en: "Casa Perini – Brut Rosé",
    },
    descricao: {
      pt: "Espumante Brut Rosê.",
      en: "Brut Rosé Sparkling Wine.",
    },
    preco: "R$ 110,00",
    imagem: "/vinhos/casa-perini-rose.webp",
  },
  {
    titulo: {
      pt: "Vinho Verde 500 – Rose",
      en: "Vinho Verde 500 - Rose",
    },
    descricao: {
      pt: "Portugal - 750 ml.",
      en: "Portugal - 750 ml.",
    },
    preco: "R$ 100,00",
    imagem: "/vinhos/vinho-verde-rose.webp",
  },
  {
    titulo: {
      pt: "Vinho Verde 500 – Branco",
      en: "Vinho Verde 500 - White",
    },
    descricao: {
      pt: "Portugal - 750 ml.",
      en: "Portugal - 750 ml.",
    },
    preco: "R$ 100,00",
    imagem: "/vinhos/vinho-verde-branco.webp",
  },
];

export const destiladosDose: Bebida[] = [
  {
    titulo: {
      pt: "Absolut",
      en: "Absolut vodka",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 19,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Orloff",
      en: "Orloff Vodka",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Campari",
      en: "Campari",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Gin Nacional",
      en: "Brazilian Gin",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 14,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Gin Beefeater",
      en: "Gin Beefeater",
    },
    descricao: {
      pt: "",
      en: "Imported Gin",
    },
    preco: "R$ 20,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Tequila José Cuervo",
      en: "Tequila José Cuervo",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 22,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Licor 43",
      en: "43 liquor",
    },
    descricao: {
      pt: "Cuarenta y tres",
      en: "Cuarenta y tres",
    },
    preco: "R$ 28,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Licor Jagermeister",
      en: "Licor Jagermeister",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 22,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Amarula",
      en: "Amarula",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 20,00",
    imagem: "/null.webp",
  },
];

export const destiladosGarrafa: Bebida[] = [
  {
    titulo: {
      pt: "Cachaça Geremia Ouro",
      en: "Cachaça Geremia Gold",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 100,00",
    imagem: "/destilados/souvenir-cachaca-ouro.webp",
  },
  {
    titulo: {
      pt: "Cachaça Geremia Prata",
      en: "Cachaça Geremia Silver",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 90,00",
    imagem: "/destilados/souvenir-cachaca-prata.webp",
  },
  {
    titulo: {
      pt: "Absolut",
      en: "Absolut",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 200,00",
    imagem: "/destilados/absolut.webp",
  },
  {
    titulo: {
      pt: "Orloff",
      en: "Orloff Vodka",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 120,00",
    imagem: "/destilados/orloff.webp",
  },
  {
    titulo: {
      pt: "Gin Orloff",
      en: "Brazilian Gin",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 110,00",
    imagem: "/destilados/gin-orloff.webp",
  },
  {
    titulo: {
      pt: "Gin Beefeater",
      en: "Imported Gin",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 220,00",
    imagem: "/destilados/beefeater.webp",
  },
  {
    titulo: {
      pt: "Campari",
      en: "Campari",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 140,00",
    imagem: "/destilados/campari.webp",
  },
  {
    titulo: {
      pt: "Tequila José Cuervo",
      en: "Tequila José Cuervo",
    },
    descricao: {
      pt: "",
      en: "",
    },
    preco: "R$ 200,00",
    imagem: "/destilados/jose-cuervo.webp",
  },
];

export const whiskyDose: Bebida[] = [
  {
    titulo: {
      pt: "Old Parr",
      en: "Old Parr",
    },
    descricao: {
      pt: "12 anos",
      en: "12 years",
    },
    preco: "R$ 25,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Chivas",
      en: "Chivas",
    },
    descricao: {
      pt: "12 anos",
      en: "12 years",
    },
    preco: "R$ 22,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Black Label",
      en: "Johnnie Walker Black Label",
    },
    descricao: {
      pt: "12 anos",
      en: "12 years",
    },
    preco: "R$ 28,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Jack Daniel’s",
      en: "Jack Daniel’s",
    },
    descricao: {
      pt: "8 anos",
      en: "8 years",
    },
    preco: "R$ 25,00",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Red Label",
      en: "Johnnie Walker Red Label",
    },
    descricao: {
      pt: "8 anos",
      en: "8 years",
    },
    preco: "R$ 20,00",
    imagem: "/null.webp",
  },
];

export const whiskyGarrafa: Bebida[] = [
  {
    titulo: {
      pt: "Old Parr",
      en: "Old Parr",
    },
    descricao: {
      pt: "12 anos",
      en: "12 years",
    },
    preco: "R$ 320,00",
    imagem: "/destilados/old-parr.webp",
  },
  {
    titulo: {
      pt: "Chivas",
      en: "Chivas",
    },
    descricao: {
      pt: "12 anos",
      en: "12 years",
    },
    preco: "R$ 290,00",
    imagem: "/destilados/chivas.webp",
  },
  {
    titulo: {
      pt: "Black Label",
      en: "Johnnie Walker Black Label",
    },
    descricao: {
      pt: "12 anos",
      en: "12 years",
    },
    preco: "R$ 350,00",
    imagem: "/destilados/black-label.webp",
  },
  {
    titulo: {
      pt: "Jack Daniel’s",
      en: "Jack Daniel's",
    },
    descricao: {
      pt: "8 anos",
      en: "8 years",
    },
    preco: "R$ 300,00",
    imagem: "/destilados/jack-daniels.webp",
  },
  {
    titulo: {
      pt: "Red Label",
      en: "Johnnie Walker Red Label",
    },
    descricao: {
      pt: "8 anos",
      en: "8 years",
    },
    preco: "R$ 200,00",
    imagem: "/destilados/jack-daniels.webp",
  },
];

export const growler: Bebida[] = [
  {
    titulo: {
      pt: "Growler de chopp Heineken",
      en: "Heineken",
    },
    descricao: {
      pt: "Chopp premium, com ingredientes puros e selecionados. 1 Litro.",
      en: "1 liter",
    },
    preco: "R$ 27,90,00",
    imagem: "/chopps/growler-heineken.webp",
  },
  {
    titulo: {
      pt: "Growler de chopp Antarctica",
      en: "Antarctica",
    },
    descricao: {
      pt: "Chopp fresco, aroma neutro. Para quem prefere chopp com teor de amargor menos acentuado. 1 Litro.",
      en: "1 liter",
    },
    preco: "R$ 21,90",
    imagem: "/null.webp",
  },
  {
    titulo: {
      pt: "Growler de chopp Amstel",
      en: "Amstel",
    },
    descricao: {
      pt: "",
      en: "1 liter",
    },
    preco: "R$ 21,90",
    imagem: "/null.webp",
  },
];
